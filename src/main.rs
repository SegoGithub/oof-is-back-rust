...
fn main() {
    ...
    crossplatform::path();
    fn main_menu() {
        console::Term::clear_screen(&console::Term::stdout()).unwrap();
        println!(
            "Your Roblox Path was detected as: {:#?}",
            crossplatform::path().0
        );
    ...
    fn handle_input(input: u32) {
        match input {
            ...
            3 => {
                ...
                let latest_version = json["tag_name"].to_string().replace("\"", "");
    ...
    fn handle_sound_input(input: u32) {
        match input {
            ...
            2 => {
                crossplatform::replace_sound(
                    "Vine Boom Sound Effect (Bass Boosted)".to_string(),
                    "vineboom".to_string(),
                );
                println!("Returning to main menu in 2 seconds..");
                std::thread::sleep(std::time::Duration::from_secs(2));
            ...
            5 => {
                crossplatform::replace_sound(
                    "Old Minecraft Death Sound".to_string(),
                    "mc".to_string(),
                );
                println!("Returning to main menu in 2 seconds..");
                std::thread::sleep(std::time::Duration::from_secs(2));
